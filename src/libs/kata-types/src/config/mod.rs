...
/// Option enabling translation of KATA_VISIBLE_DEVICES into CDI GPU requests
pub const KATA_VISIBLE_DEVICES_OPTION: &str = "agent.kata_visible_devices";
...
impl TomlConfig {
    ...
    pub fn get_agent_kernel_params(&self) -> Result<BTreeMap<String, String>> {
        ...
        if let Some(cfg) = self.agent.get(&self.runtime.agent_name) {
            ...
            if cfg.kata_visible_devices {
                kv.insert(KATA_VISIBLE_DEVICES_OPTION.to_string(), "true".to_string());
            }
...
mod tests {
    ...
    fn test_get_agent_kernel_params() {
        ...
        assert_eq!(kv.get("agent.kata_visible_devices").unwrap(), "true");
    }
}
