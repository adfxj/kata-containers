...
use super::vsock_uds_forward::parse_vsock_uds_forward;
...
pub struct Runtime {
    ...
    /// Bridge a guest vsock listener to a host unix socket (`port:/absolute/unix/path`).
    #[serde(default)]
    pub vsock_uds_forward: String,
}
...
impl ConfigOps for Runtime {
    ...
    fn validate(conf: &TomlConfig) -> Result<()> {
        ...
        if let Err(err) = parse_vsock_uds_forward(&conf.runtime.vsock_uds_forward) {
