...
use crate::device::{
    add_devices, cdi_devices_from_kata_visible_devices, dump_nvidia_cdi_yaml, handle_cdi_devices,
    update_env_pci,
};
...
impl AgentService {
    ...
    async fn do_create_container(
        ...
        // When enabled, translate the container's KATA_VISIBLE_DEVICES
        ...
        let kata_visible_devices = if AGENT_CONFIG.kata_visible_devices {
            cdi_devices_from_kata_visible_devices(&oci)?
        } else {
            Vec::new()
        };
        handle_cdi_devices(
