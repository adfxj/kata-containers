...
enum Error {
    ...
    #[error("Error parsing patch filesystem syntax")]
    PatchFsConfig(#[source] vmm::config::Error),
...
fn dbus_api_do_command(matches: &ArgMatches, proxy: &DBusApi1ProxyBlocking<'_>) -> ApiResult {
    match matches.subcommand_name() {
        ...
        Some("patch-fs") => {
            ...
            proxy.api_vm_patch_fs(&patch_fs_config)
        }
        Some("add-generic-vhost-user") => {
...
fn patch_fs_config(config: &str) -> Result<String, Error> {
    let patch_fs_config = vmm::vm_config::FsMountConfigInfo::parse(config).map_err(Error::PatchFsConfig)?;
