...
pub enum Error {
    ...
    #[error("Error manipulate backend fs")]
    ManipulateBackendFs,

    #[error("Error restore backend fs")]
    RestoreBackendFs,
...
impl Vm {
    ...
    pub fn manipulate_fs_backend_fs(&mut self, config: FsMountConfigInfo) -> Result<()> {
            ...
            if let Some(virtio_fs_device) = device.downcast_mut::<virtio_devices::VirtioFs>() {
                if virtio_fs_device.get_tag() == config.tag {
                    ...
                    if let Err(_) = virtio_fs_device.manipulate_backend_fs(
                        ...
                        error!("Failed to manipulate backend fs for tag: {}", config.tag);
                        return Err(Error::ManipulateBackendFs);
                    }
                    break;
                }
            }
        ...
        if !found {
            error!("fs tag {} id not found", config.tag);
    ...
    pub fn restore_backend_fs(&mut self, config: FsMountConfigInfo, fuse_backend_init: bool) -> Result<()> {
        let mut found = false;
        let device_manager = self.device_manager.lock().unwrap();
        let virtio_devices = &device_manager.virtio_devices;
        ...
        for virtio_device in virtio_devices.iter() {
            ...
            if let Some(virtio_fs_device) = device.downcast_mut::<virtio_devices::VirtioFs>() {
                if virtio_fs_device.get_tag() == config.tag {
                    found = true;
                    let id = virtio_fs_device.get_id();
                    if let Err(_) = virtio_fs_device.restore_backend_fs(
                        config.old_source.clone(),
                        config.source.clone(),
                        config.fstype.clone(),
                        &config.mountpoint,
                        config.config.clone(),
                        ...
                        config.dax_threshold_size_kb,
                        fuse_backend_init,
                        state_from_id(device_manager.snapshot.as_ref(), &id)
                            .map_err(|e| {
                                error!("Failed to get VirtioFsState from Snapshot: {}", e);
                                Error::RestoreBackendFs
                            })?,
                    ) {
                        error!("Failed to restore backend fs for tag: {}", config.tag);
                        return Err(Error::RestoreBackendFs);
                    }
                    break;
                }
            }
    ...
    pub fn get_fuse_backend_init(&mut self, tag: &str) -> Result<bool> {
        let device_manager = self.device_manager.lock().unwrap();
        let virtio_devices = &device_manager.virtio_devices;
        ...
        for virtio_device in virtio_devices.iter() {
            let mut device = virtio_device.virtio_device.lock().unwrap();
            if let Some(virtio_fs_device) = device.downcast_mut::<virtio_devices::VirtioFs>() {
                if virtio_fs_device.get_tag() == tag {
                    return Ok(virtio_fs_device.get_fuse_backend_init());
                }
            }
        }
