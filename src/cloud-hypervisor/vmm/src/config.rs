...
pub enum Error {
    ...
    #[error("Error parsing --patch-fs")]
    ParsePatchFileSystem(#[source] OptionParserError),
...
pub enum ValidationError {
    ...
    #[error("Native virtio-fs shouldn't have socket argument")]
...
impl<'a> VmParams<'a> {
    pub fn from_arg_matches(args: &'a ArgMatches) -> Self {
        ...
        let fs: Option<Vec<&str>> = args
            .get_many::<String>("fs")
            .map(|x| x.map(|y| y as &str).collect());
        let patch_fs: Option<Vec<&str>> = args
            .get_many::<String>("patch-fs")
            .map(|x| x.map(|y| y as &str).collect());
...
impl FsConfig {
    ...
    pci_segment=<segment_id>,pci_device_id=<pci_slot>,\
    native=on|off,thread_pool_size=<thread_pool_size>,
    cache=auto|always|none|metadata,writeback_cache=on|off,\
    no_open=on|off,killpriv_v2=on|off,no_readdir=on|off,\
    xattr=on|off,drop_sys_resource=on|off\"";
...
impl RestoreConfig {
    ...
    pub fn parse(restore: &str) -> Result<Self> {
        ...
        let fs_sources = parser
            ...
            .map(|v| {
                v.0.iter()
                    .map(|(tag, source)| RestoreFsConfig {
                        tag: tag.clone(),
                        source: source.clone(),
                    })
                    .collect()
...
impl VmConfig {
    ...
    pub fn parse(vm_params: VmParams) -> Result<Self> {
        ...
        let mut patch_fs: Option<Vec<FsMountConfigInfo>> = None;
        if let Some(fs_list_mount_config) = &vm_params.patch_fs {
            let mut fs_mount_config_list = Vec::new();
            for item in fs_list_mount_config.iter() {
                fs_mount_config_list.push(FsMountConfigInfo::parse(item)?);
            }
            patch_fs = Some(fs_mount_config_list);
        }
