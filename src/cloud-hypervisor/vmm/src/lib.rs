...
use crate::vm_config::{
    ...
    UserDeviceConfig, VdpaConfig, VmConfig, VsockConfig, FsMountConfigInfo,
};
...
struct VmMigrationConfig {
    ...
    fuse_backend_inits: HashMap<String, bool>,
}
...
impl Vmm {
    ...
    fn vm_receive_migration_step(
        ...
        let mut fuse_backend_inits = HashMap::new();

        let mut configure_vm =
            |socket: &mut SocketStream,
             memory_files: HashMap<u32, File>,
             fuse_backend_inits: &mut HashMap<String, bool>|
             -> std::result::Result<ReceiveMigrationConfiguredData, MigratableError> {
                let memory_manager = self.vm_receive_config(req, socket, fuse_backend_inits,memory_files)?;
        ...
        match state {
            ...
            Started => match req.command() {
                ...
                Command::Config => configure_vm(socket, Default::default(), &mut fuse_backend_inits).map(Configured),
                _ => invalid_command(),
            },
            MemoryFdsReceived(memory_files) => match req.command() {
                ...
                Command::Config => {
                    configure_vm(socket, HashMap::from_iter(memory_files), &mut fuse_backend_inits).map(Configured)
                }
                _ => invalid_command(),
            },
            Configured(mut config_data) => match req.command() {
                ...
                Command::State => {
                    ...
                    let (recv_state_dur, restore_vm_dur) =
                        self.vm_receive_state(req, socket, config_data.memory_manager, fuse_backend_inits.clone())?;
                    debug!(
        ...
        T: Read,
    {
        ...
        fuse_backend_inits.clone_from(&vm_migration_config.fuse_backend_inits);
        ...
        T: Read,
    {
        ...
        let (vm, restore_duration) = measure_ok(|| {
            ...
            if let Some(fs_mount_list_config) = &self.vm_config.clone().unwrap().lock().unwrap().patch_fs {
                for fs_mount_config in fs_mount_list_config.iter() {
                    for (tag, fuse_backend_init) in &fuse_backend_inits {
                        if *tag == fs_mount_config.tag {
                            vm.restore_backend_fs(fs_mount_config.clone(), fuse_backend_init.clone()).map_err(|e| {
                                MigratableError::MigrateReceive(anyhow!("Error restoring fs backend: {:?}", e))
                            })?;
                        }
                    }
                }
    ...
    fn send_migration(
        ...
        let mut fuse_backend_inits = HashMap::new();
        if let Some(fs_configs) = &vm_config.lock().unwrap().fs {
            for fs_config in fs_configs.iter() {
                let fuse_backend_init = vm.get_fuse_backend_init(&fs_config.tag).unwrap();
                fuse_backend_inits.insert(fs_config.tag.clone(), fuse_backend_init);
            }
        }
        #[cfg(all(feature = "kvm", target_arch = "x86_64"))]
        let common_cpuid = {
    ...
    fn vm_restore(
        ...
        if let Some(fs_mount_list_config) = &vm_config.clone().lock().unwrap().patch_fs {
            for fs_mount_config in fs_mount_list_config.iter() {
                vm.restore_backend_fs(fs_mount_config.clone(), fs_backend_init)?;
            }
        }
...
impl RequestHandler for Vmm {
            ...
            if self.vm.is_none() {
                ...
                if let Some(ref vm_config) = self.vm_config {
                    ...
                    if let Some(fs_mount_list_config) = &vm_config.lock().unwrap().patch_fs {
                        for fs_mount_config in fs_mount_list_config.iter() {
                            vm.manipulate_fs_backend_fs(fs_mount_config.clone())?;
                        }
                    }
    ...
    fn vm_restore(&mut self, restore_cfg: RestoreConfig) -> result::Result<(), VmError> {
        ...
        // Update VM's fs configurations with new fs source dir
        if let (Some(fs_sources), Some(fs_mount_list_config)) =
            (restore_cfg.fs_sources, &mut vm_config.lock().unwrap().patch_fs)
        {
            for fs_source in fs_sources.iter() {
                for fs_mount in fs_mount_list_config.iter_mut() {
                    if fs_source.tag.clone() == fs_mount.tag.clone() {
                        fs_mount.old_source.clone_from(&fs_mount.source.clone());
                        fs_mount.source.clone_from(&Some(fs_source.source.clone()));
                    }
                }
            }
        }
        ...
        // Update VM's vsock socket dir with new socket path
        if let (Some(restore_vsock_socket), Some(vm_vsock)) = 
