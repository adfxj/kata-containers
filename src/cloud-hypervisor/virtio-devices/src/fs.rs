...
use crate::{
    ...
    EPOLL_HELPER_EVENT_LAST, VIRTIO_F_ACCESS_PLATFORM, VIRTIO_F_VERSION_1,
};
...
use fuse_backend_rs::passthrough::{CachePolicy, Config as PassthroughConfig, PassthroughFs, SnapshotStore};
...
pub struct State {
    ...
    pub vfs_bytes: Vec<u8>,
    pub backend_fs: HashMap<String, BackendFsInfo>,
    pub vfs_snapshot: HashMap<u8, SnapshotStore>,
    pub capable: u64,
}
...
impl VirtioFs {
    ...
    pub fn new(
        ...
        let (avail_features, acked_features, config, restore_vfs_bytes, paused) = if let Some(state) = state {
            info!("Restoring virtio-fs {}", id);
            (
                state.avail_features,
                state.acked_features,
                state.config,
                Some(state.vfs_bytes),
                true,
            )
        } else {
            ...
            (avail_features, 0, config, None, false)
        };
        ...
        let fs = Arc::new(Vfs::new(vfs_opts));
        if let Some(mut vfs_bytes) = restore_vfs_bytes {
            // restore bytes
            fs.restore_from_bytes(&mut vfs_bytes)
            .map_err(|e| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("Invalid restore vfs bytes {e}"),
                )
            })?;
        }
    ...
    fn state(&self) -> State {
        let mut vfs_snapshot = HashMap::new();
        for (_path, backend_info) in self.backend_fs.clone().iter() {
            match self.fs.save_filesystem(backend_info.index) {
                Ok(mut snapshot_store) => {
                    snapshot_store.deduplicate_inode();
                    vfs_snapshot.insert(backend_info.index, snapshot_store);
                }
                Err(e) => {
                    error!("Failed to get filesystem for index {} with error: {:?}", backend_info.index, e);
                }
            }
        }
        State {
            ...
            config: self.config,
            vfs_bytes: self.fs.save_to_bytes().unwrap(),
            backend_fs: self.backend_fs.clone(),
            vfs_snapshot,
            capable: self.fs.get_capable(),
        }
    }

    fn get_timeout(&self) -> Duration {
        match self.backendfs_config.cache {
            1 => Duration::from_secs(CACHE_ALWAYS_TIMEOUT),
            2 => Duration::from_secs(CACHE_NONE_TIMEOUT),
            0 => Duration::from_secs(CACHE_AUTO_TIMEOUT),
            3 => Duration::from_secs(CACHE_ALWAYS_TIMEOUT),
            4_u8..=u8::MAX => Duration::from_secs(CACHE_NONE_TIMEOUT),
        }
    }
    ...
    pub fn restore_backend_fs(
        ...
        dax_threshold_size_kb: Option<u64>,
        fuse_backend_init: bool,
        state: Option<State>,
    ) -> Result<()> {
        info!("Restore backend fs.");

        match fstype.as_deref() {
            Some("PassthroughFs") | Some(PASSTHROUGHFS) => {
                if fuse_backend_init {
                    if let Some(restore_state) = state {
                        ...
                        // restore mount
                        for (path, backend_info) in restore_state.backend_fs.iter() {
                            let Some((source_path, _)) = backend_info.src_cfg.as_ref() else {
                                continue;
                            };

                            let should_continue = match old_source.as_ref() {
                                Some(old_path) => old_path.clone() != *source_path && source.clone() != *source_path,
                                None => source.clone() != *source_path,
                            };
                            ...
                            let passthrough_fs = PassthroughFs::<()>::new(fs_cfg.clone()).map_err(Error::IOError)?;

                            if let Some(snapshot_store) = restore_state.vfs_snapshot.get(&backend_info.index) {
                                let capable = FsOptions::from_bits_truncate(restore_state.capable);
                                passthrough_fs.do_restore(snapshot_store.clone(), capable).map_err(|e| {
                                    error!("failed to restore passthrough fs snapshot source: {:?} error: {:?}", source, e);
                                    e
                                })?;
                            }

                            let fs: Box<PassthroughFs> = Box::new(passthrough_fs);
                            self.fs.restore_mount(fs, backend_info.index, path)?;
                            self.backend_fs.insert(
                            path.to_string(),
                            BackendFsInfo {
                                index: backend_info.index,
                                fstype: PASSTHROUGHFS.to_string(),
                                src_cfg: Some((source.clone(), source.clone())),
                                },
                            );
                        }
                    }
                } else {
                    self.manipulate_backend_fs(
                        source,
                        fstype,
                        mountpoint,
                        config,
    ...
    pub fn get_fuse_backend_init(&mut self) -> bool {
        self.fs.initialized()
    }
}
        ...
        for i in 0..queues.len() {
            ...
            let cache_handler = if let Some(cache) = self.cache.as_ref() {
                let handler = CacheHandler {
                    cache_size: cache.0.mapping.clone().as_ref().len().try_into().unwrap(),
                    mmap_cache_addr: cache.0.mapping.clone().as_ref().as_ptr() as u64,
                    id: self.id.clone(),
                };
