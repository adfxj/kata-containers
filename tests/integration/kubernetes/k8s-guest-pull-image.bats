...
setup() {
    ...
    unencrypted_image_run_as_user=""
    unencrypted_image_run_as_group=""
    unencrypted_image_supplemental_groups=""
    large_image_run_as_user=""
    large_image_run_as_group=""
    large_image_supplemental_groups=""
    if auto_generate_policy_enabled; then
        unencrypted_image_run_as_user=0
        unencrypted_image_run_as_group=0
        unencrypted_image_supplemental_groups="10"
        large_image_run_as_user=0
        large_image_run_as_group=0
...
@test "Test we can pull an unencrypted image outside the guest with runc and then inside the guest successfully" {
    ...
    kata_pod_with_nydus_config="$(new_pod_config "$unencrypted_image" "kata-${KATA_HYPERVISOR}" \
        "$unencrypted_image_run_as_user" "$unencrypted_image_run_as_group" "$unencrypted_image_supplemental_groups")"
...
@test "Test we cannot pull a large image that pull time exceeds createcontainer timeout inside the guest" {
    ...
    ! auto_generate_policy_enabled || set_pod_spec_security_context "$pod_config" ".spec" \
        "$large_image_run_as_user" "$large_image_run_as_group" "$large_image_supplemental_groups"
