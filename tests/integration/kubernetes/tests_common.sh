...
set_nginx_image() {
	...
	case "$(yq -r 'select(documentIndex == 0) | .kind' "${output_yaml}")" in
		Pod)
			set_pod_spec_security_context "${output_yaml}" ".spec" 0 0 "1, 2, 3, 4, 6, 10, 11, 20, 26, 27"
			;;
		Deployment|ReplicationController)
			set_pod_spec_security_context "${output_yaml}" ".spec.template.spec" 0 0 "1, 2, 3, 4, 6, 10, 11, 20, 26, 27"
			;;
	esac
}
